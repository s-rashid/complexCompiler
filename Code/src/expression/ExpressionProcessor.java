...
public class ExpressionProcessor {
		...
		for (Expression e : elist) {
			...
			if (e instanceof Variable_Declaration_no_type) {
				...
				if (this.array_values.containsKey(input.id + input.pos)) {
					String value_in_array = "(assert (= (store " + input.id + " " + input.pos + " "
							+ this.array_values.get(input.pos) + " ) " + input.id + "))";
				...
				if (this.array_values_string.containsKey(input.id + input.pos)) {
					String value_in_array = "(assert (= (store " + input.id + " " + input.pos + " "
							+ this.array_values_string.get(input.pos) + " ) " + input.id + "))";
					for (int i = 0; i < this.final_input.size(); i++) {
						String from_final_input_list = this.final_input.get(i).trim();
						// System.out.println("THE VALUE IN ARRAY IS" + from_final_input_list );
						// System.out.println("THE FROM FINAL INPUT LSIT IS" + from_final_input_list);
					...
					|| e instanceof Bool_greater || e instanceof Bool_less_then || e instanceof Bool_and
					|| e instanceof Bool_or || e instanceof Bool_not || e instanceof Bool_equal
					|| e instanceof FunctionInvocWithPrama || e instanceof FuncInvocWithoutParam) {
				String input = e.toString();
				...
				// System.out.println("sss " + counter + "the list size is " +
				// this.list.size());
			}
				...
				String result = getEvalResult(e);
				String[] result_split = result.split("split2");
				String final_result = "(assert ";
				final_result = final_result + result_split[1];
				final_result = final_result + " )";
				this.final_input.add(final_result);
			} else {
	...
	private String getEvalResult(Expression e) {
		...
		if (e instanceof Number) {
		...
		} else if (e instanceof array_declaration_expr_id_position) {
			array_declaration_expr_id_position var = (array_declaration_expr_id_position) e;
			String values_map = this.values.get(var.pos);
			result = this.array_values.get(var.id + values_map);
			System.out.println("The result is 322 " + result);
		} else if (e instanceof array_declaration_expr) {
			array_declaration_expr var = (array_declaration_expr) e;
			result = this.array_values.get(var.id + var.pos);
		} else if (e instanceof nested_if_assignment_else) {
			nested_if_assignment_else num = (nested_if_assignment_else) e;
			String else_condition = getEvalResult(num.else_expression);
			String true_condition = num.ArrayListToString();
			...
			if (else_condition.toString().contains("assert")) {
				else_condition = else_condition.replace("assert", "");
				else_condition = else_condition.substring(1, else_condition.length() - 1);
			}
		...
		} else if (e instanceof if_condition_to_assignment2) {
			if_condition_to_assignment2 var = (if_condition_to_assignment2) e;
			String else_expression = var.toString();
			if (else_expression.contains("assert")) {
				else_expression = else_expression.replace("assert", "");
				else_expression = else_expression.substring(1, else_expression.length() - 2);
			}
			result = var.condition + "!" + var.case_true + "split2" + else_expression;
			...
			// System.out.println("The split is " +else_condition_split[1] );
			if (else_condition_split.length == 1) {
			...
			if (post_split[0].equals("=")) {
				...
				if (math_split[1].equals("+")) {
					if ((math_split[2].startsWith("0") || math_split[2].startsWith("1") || math_split[2].startsWith("2")
							|| math_split[2].startsWith("3") || math_split[2].startsWith("4")
							|| math_split[2].startsWith("5") || math_split[2].startsWith("6")
							|| math_split[2].startsWith("7") || math_split[2].startsWith("8")
							|| math_split[2].startsWith("9"))
							&& (post_split[2].startsWith("0") || post_split[2].startsWith("1")
									|| post_split[2].startsWith("2") || post_split[2].startsWith("3")
									|| post_split[2].startsWith("4") || post_split[2].startsWith("5")
									|| post_split[2].startsWith("6") || post_split[2].startsWith("7")
									|| post_split[2].startsWith("8") || post_split[2].startsWith("9"))) {
						try {
						...
						post_new = "( " + post_split[0] + " " + math_split[0] + " (- " + before_equal + " "
								+ after_equal + "))";
						// System.out.println("Here");
					}
				}
				...
				if (math_split[1].equals("*")) {
					if ((math_split[2].startsWith("0") || math_split[2].startsWith("1") || math_split[2].startsWith("2")
						...
						String before_equal = math_split[2];
						String after_equal = post_split[2];
						post_new = "( " + post_split[0] + " " + math_split[0] + " (div " + before_equal + " "
								+ after_equal + "))";
						// System.out.println("Here");
					}
				}
			...
			if (post_split[0].equals("and")) {
				...
				post_new = "(and " + "(" + bool_split[1] + " " + bool_split[0] + " " + bool_split[2] + ")" + " "
						+ post_split[2] + ")";
			...
			if (post_split[0].equals("or")) {
				String[] bool_split = new_post_split_1.split(" ");
				post_new = "(or " + "(" + bool_split[1] + " " + bool_split[0] + " " + bool_split[2] + ")" + " "
						+ post_split[2] + ")";
			...
			if (true_expression.contains("assert")) {
				...
				true_expression = true_expression.substring(2, true_expression.length() - 1);
			}
			...
			result = num.if_expression + "!" + true_expression + "!" + else_expression + "split2" + "(ite "
					+ num.if_expression + " " + true_expression + " " + else_expression + " )";
		}
			...
			if (imple_class_type.equals("class expression.if_else_expr")) {
				...
				if (imple_split_else[1].equals("=")) { // formating in the terms of y = id (= or - or div) id
					...
					if (imple_split_else.length > 4 && imple_split_else.length <= 7) {
						...
						new_imple_else += " " + imple_split_else[4] + " " + imple_split_else[3] + " "
								+ imple_split_else[5];
				...
				String new_post_split_1_else = post_split_else[1].replace(new_imple_split_else[0].trim(),
						new_imple_split_else[1].trim());
				...
				if (post_split[0].equals("=")) {
					...
					if (math_split_else.length > 1) {
						...
						if (math_split_else[1].equals("+")) {
							if ((math_split_else[2].startsWith("0") || math_split_else[2].startsWith("1")
									|| math_split_else[2].startsWith("2") || math_split_else[2].startsWith("3")
									|| math_split_else[2].startsWith("4") || math_split_else[2].startsWith("5")
									|| math_split_else[2].startsWith("6") || math_split_else[2].startsWith("7")
									|| math_split_else[2].startsWith("8") || math_split_else[2].startsWith("9"))
									&& (post_split_else[2].startsWith("0") || post_split_else[2].startsWith("1")
											|| post_split_else[2].startsWith("2") || post_split_else[2].startsWith("3")
											|| post_split_else[2].startsWith("4") || post_split_else[2].startsWith("5")
											|| post_split_else[2].startsWith("6") || post_split_else[2].startsWith("7")
											|| post_split_else[2].startsWith("8")
											|| post_split_else[2].startsWith("9"))) {
								try {
								...
								post_new_else = "( " + post_split_else[0] + " " + math_split_else[0] + " (- "
						...
						if (math_split_else[1].equals("*")) {
							if ((math_split_else[2].startsWith("0") || math_split_else[2].startsWith("1")
								...
								String before_equal = math_split_else[2];
								String after_equal = post_split_else[2];
								post_new_else = "( " + post_split_else[0] + " " + math_split_else[0] + " (div "
										+ before_equal + " " + after_equal + "))";
								// System.out.println("Here");
							}
						}
						...
						post_new_else = "( " + post_split_else[0] + " " + new_post_split_1_else + " "
								+ post_split_else[2] + " )";
					...
					if (math_split.length > 1) {
						...
						if (math_split[1].equals("+")) {
							if ((math_split[2].startsWith("0") || math_split[2].startsWith("1")
									|| math_split[2].startsWith("2") || math_split[2].startsWith("3")
									|| math_split[2].startsWith("4") || math_split[2].startsWith("5")
									|| math_split[2].startsWith("6") || math_split[2].startsWith("7")
									|| math_split[2].startsWith("8") || math_split[2].startsWith("9"))
									&& (post_split[2].startsWith("0") || post_split[2].startsWith("1")
											|| post_split[2].startsWith("2") || post_split[2].startsWith("3")
											|| post_split[2].startsWith("4") || post_split[2].startsWith("5")
											|| post_split[2].startsWith("6") || post_split[2].startsWith("7")
				...
				// ))))";
				// result = ("( assert (not( => (and" + pre + " (" + imple_pre + ") ) " +
				// post_new + " )))");
			...
			// System.out.println("The size of imple_new_line_split is " +
			// imple_new_line_split.length);
			...
			if (this.values.containsKey(left) && this.values.containsKey(right)) {
				...
				// check if float
				try {
					System.out.println("the left value is " + left);
					Float x = Float.parseFloat(left);
					Float x1 = x + Float.parseFloat(this.values.get(right));
					result = String.valueOf(x1);
				} catch (NumberFormatException e1) {
		...
		/*
		 * This part of the code evaluates boolean expressions '>', '<', '>=', '<=' and
		 * '=='.
		 */
